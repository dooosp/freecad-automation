...
| Ubuntu hosted CI (`ubuntu-24.04`) | N/A for live FreeCAD | No | Hosted-safe only | Hosted CI runs Node contract, Node integration, snapshots, and Python lanes without installing FreeCAD; the Node contract lane includes the non-mutating Stage 5B audit CLI smoke with GitHub discovery disabled. |
...
- Hosted CI covers Node runtime/path contracts, the non-mutating Stage 5B audit CLI smoke, non-runtime integration checks, snapshots, and Python tests without claiming a live FreeCAD install.
