...
6. For Stage 5B evidence work, run `fcad inspection-evidence-intake --out <report.json>` to emit a machine-readable discovery report. Use `--include-github` only for bounded public GitHub discovery; it searches non-secret issues, PR/comments, releases/assets, workflow artifact metadata, and allowlisted public links, then records machine-readable skip/download/rejection provenance. The intake adapter checks JSON contract files plus explicit CSV/TSV/Markdown/TXT inspection tables, including safety-checked allowlisted ZIP contents, while rejecting generated quality/readiness/review/release artifacts, CI summaries, GitHub metadata alone, and fixtures as final evidence. Then run `fcad inspection-evidence-promotion-dry-run --intake-report <report.json> --out <promotion_dry_run_manifest.json>` or queue `inspection-evidence-promotion-dry-run` from a registered tracked `inspection-evidence.intake-report` artifact in Studio Review to write a deterministic non-mutating manifest with exact future commands, expected outputs, mutation boundaries, blockers, readiness expectation, and rollback guidance. The tracked result is `inspection-evidence.promotion-dry-run-manifest`. When intake finds no genuine completed attachment-ready inspection evidence, the dry-run manifest keeps readiness at `needs_more_evidence` / `hold_for_evidence_completion`.
