...
- Hosted CI covers Node runtime/path contracts, the non-mutating Stage 5B no-evidence CLI lane, audit CLI smoke, surrogate inspection validation guard, non-runtime integration checks, snapshots, and Python tests without claiming a live FreeCAD install.
