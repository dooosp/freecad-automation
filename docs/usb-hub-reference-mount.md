...
**현재 구성과 범위:** 사용자는 2026-09-17 실물 없이 기준 설계로 진행한다고 확인했다. [통합 기준 설계 패키지](usb-hub-reference-design-package.md)는 장착판 R1과 M3×10 조립 형상 후보를 도면·부품표·검토 근거에 연결한다. 아래 원본 R1 조립과 과거 검토는 이력으로 보존한다. 실물 시험은 현재 범위 밖의 향후 단계다.
...
원본 R1 조립은 허브 M3×12와 평와셔/너트, 받침면 M5×25와 평와셔/너트/스페이서의 공칭 외형으로 검토했다. 현재 통합 패키지는 별도로 검증한 M3×10 형상 후보를 참조한다. 실제 체결재 강도 등급, 토크, 풀림 방지와 제조 공차는 확정하지 않았다. M3용 Ø4.0은 별도의 설계 선택이며 표준 중간 여유 구멍이라는 주장이 아니다.
...
향후 물리 검토 단계는 [장착·조작 하중·열 시험 계획](usb-hub-physical-test-plan.md)에 정리했다. 실제 ChatGPT 6 Pro 자문과 1차 자료 대조를 반영했으며, [빈 계획용 기록 양식](templates/usb-hub-physical-test.template.json)의 시험하중·토크·측정값은 실물 입력이 확보될 때 채운다. 현재 실물 시험 상태는 `not_tested`다.

[체결 길이·좌면·하중 입력 검토](usb-hub-reference-joint-review.md)에서는 8개 길이 샘플을 실제 CAD로 비교하고 M3×10을 별도 형상 후보로 다뤘다. 기존 R1의 M3×12 원본 파일을 보존하며, 판 두께만의 민감도와 전체 공차 보증값을 구분한다.

[기준 설계 검토 마무리](usb-hub-reference-closeout.md)에 제조사 체결품 후보, 상·하 좌면 비교, 최종 Pro 자문 반영, 재현 자료와 물리 판단 재개 조건을 모았다. 자동으로 가능한 기준 검토·문서 준비를 종료했으며 실제 장착·하중·열 시험과 제조 승인은 미완료다.

[공칭 편심 추가 검토](usb-hub-reference-eccentricity.md)는 중심 이동에 따른 지지 평면의 잘림과 접선 간격 0을 다룬다. 기준 평와셔와 넓은 비교 와셔에 같은 면적 불변 결론을 적용하지 않는다.
