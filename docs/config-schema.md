...
Each selected member must exist in the measured hole set and match the declared diameter. Evidence contains only that group's face references and centers. The renderer uses the verified XY centers to select an in-group top-view circle, including non-first holes and disjoint same-diameter subgroups. Only an automatic diameter annotation with the same center and exact measured diameter can suppress the group's label. An equal numeric value on a different hole or another dimension category cannot suppress it.

Explicit groups without verified centers, unsupported projections, and projected circles with the wrong diameter receive no fallback anchor or value-only duplicate suppression. The final traceability check still requires a unique current SVG anchor inside the measured group. Unscoped mixed sizes, missing holes, open edge notches, invalid groups, wrong values, and ambiguous geometry remain unverified. A missing hole does not discard independent measured evidence for the body's bounds. Face references in `<name>_traceability.json` identify this run's topology; they are not stable revision identifiers.
