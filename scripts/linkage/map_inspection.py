...
def _build_signal_feature_refs(result, geometry_intelligence):
    ...
    signal_tokens = set(tokenize(result.get("dimension_name")))
    signal_tokens.update(tokenize(result.get("feature_hint")))
    signal_tokens.update(tokenize(result.get("location_hint")))

    if (features.get("hole_like_feature_count") or 0) > 0 and "hole" in signal_tokens:
        refs.append("hole")
    if features.get("thin_wall_candidates") and ("wall" in signal_tokens or "thickness" in signal_tokens):
        refs.append("thin_wall")

    for feature in derived_features:
        feature_type = feature.get("feature_type")
        match_tokens = set(feature.get("match_tokens") or [])
        if not (signal_tokens & match_tokens):
            continue
        if feature_type == "hole_pattern":
            refs.extend(["hole", "pattern"])
        elif feature_type == "thin_wall_candidate":
            refs.append("thin_wall")
        elif feature_type == "deep_pocket_candidate":
            refs.append("tool_access")
        elif feature_type == "inner_corner_risk":
            refs.append("inner_corner")
