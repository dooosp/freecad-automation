...
def run(config):
    """Run deep TechDraw projectEx probe and return result payload."""
    ...
    if not shapes:
        raise ValueError("No shapes configured for TechDraw projectEx test")

    final_shape = shapes[list(shapes.keys())[-1]]
    bbox = final_shape.BoundBox
    log(
        f"Bracket: {len(final_shape.Faces)}F, {len(final_shape.Edges)}E, "
            ...
            for edge in shape.Edges:
                ...
                if ctype == "Line" or ctype == "LineSegment":
                    ...
                    if abs(edge.LastParameter - edge.FirstParameter - 2 * 3.14159) < 0.01:
                        svg_parts.append(
                            f'    <circle cx="{center.x:.2f}" cy="{-center.z:.2f}" '
                            f'r="{radius:.2f}"/>'
                        )
                    else:
    ...
    def t5():
        ...
        sig = (
            str(inspect.signature(TechDraw.exportSVGEdges))
