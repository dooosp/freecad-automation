...
def _build_data_quality_notes(metadata, uncertainty_report):
    ...
    for missing in uncertainty_report.get("missing_inputs") or []:
        notes.append({
            "severity": "info",
            "message": f"Missing or limited {missing.replace('_', ' ')}; review-pack remains usable with English fallback and partial evidence.",
        })
...
def build_markdown_sections(payload_or_report):
    ...
    if prioritized_hotspots:
        for item in prioritized_hotspots[:5]:
            ...
            lines.append(f"- {priority} {item.get('category')}: score {item.get('score')} from {item.get('title')}.{action_note}")
    else:
    ...
    if actions:
        for action in actions:
            lines.append(f"- {action.get('category')}: {action.get('recommended_action')}")
    else:
