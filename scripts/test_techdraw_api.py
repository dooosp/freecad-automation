...
try:
    ...
    log(f"Bracket built: {final_shape.ShapeType}, {len(final_shape.Faces)} faces, {len(final_shape.Edges)} edges")
    ...
    def t4():
        v = state["view_front"]
        edge_attrs = sorted([a for a in dir(v) if any(k in a.lower()
                     for k in ['edge', 'visible', 'hidden', 'svg', 'geom',
                                'cosmetic', 'center', 'vertex', 'format'])])
        all_props = v.PropertiesList
        interesting = [p for p in all_props if any(k in p.lower()
                       for k in ['edge', 'visible', 'hidden', 'hard', 'smooth',
                                  'seam', 'iso', 'cosmetic', 'center', 'line'])]
    ...
    def t5():
        ...
        for method in ['getVisibleEdges', 'getHiddenEdges', 'getEdges',
                       'getVisibleLines', 'getHiddenLines',
                       'makeSvg', 'getSVG', 'getSvg']:
            if hasattr(v, method) and callable(getattr(v, method)):
                try:
                    result = getattr(v, method)()
                    info[method] = f"returned {type(result).__name__}, len={len(result) if hasattr(result, '__len__') else '?'}"
                except Exception as e:
                    info[method] = f"callable but error: {e}"
            elif hasattr(v, method):
                info[method] = f"property (not callable)"

        # Check property values for edge counts
        for prop in ['HardHidden', 'SmoothVisible', 'SeamVisible',
                     'IsoVisible', 'HardVisible', 'SmoothHidden',
                     'SeamHidden', 'IsoHidden', 'CoarseView']:
            if hasattr(v, prop):
    ...
    def t7():
        ...
        props = sorted([p for p in dim.PropertiesList if any(k in p.lower()
                 for k in ['type', 'reference', 'format', 'value',
                           'measure', 'arbitrary', 'over', 'under'])])
    ...
    def t10():
        ...
        with zipfile.ZipFile(fcstd_path, 'r') as z:
            ...
            for sf in svg_files:
                ...
                # Save extracted SVG for inspection
