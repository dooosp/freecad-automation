...
const diagnosticStages = result.runtimeDiagnostics.map((diagnostic) => diagnostic.stage);
assert.equal(diagnosticStages.includes('model-inspection'), true);
assert.equal(diagnosticStages.includes('step-feature-detection'), true);
assert.equal(diagnosticStages.includes('metadata-only-fallback'), true);
assert.equal(result.runtimeDiagnostics.length >= 3, true);
assert.equal(result.runtimeDiagnostics.length <= 4, true);
