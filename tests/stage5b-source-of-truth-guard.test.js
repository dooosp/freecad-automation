...
assert.match(docs.finalMaintainerHandoff, /PR \[#167\]/, 'final maintainer handoff should cite PR #167');
assert.match(docs.finalMaintainerHandoff, /be93a51808080f951fc155f1fab36c10f13e7f52/, 'final maintainer handoff should pin the refreshed handoff head');
assert.match(docs.finalMaintainerHandoff, /27062840652/, 'final maintainer handoff should record the refreshed post-merge hosted CI run');
assert.match(docs.finalMaintainerHandoff, /27062881118/, 'final maintainer handoff should record the refreshed post-merge self-hosted runtime smoke run');
assert.match(docs.finalMaintainerHandoff, /27078229830/, 'final maintainer handoff should record the manual maintainer-doctors dispatch run');
...
assert.match(docs.finalMaintainerHandoff, /bootstrap doctor is closed through PR #165/, 'final maintainer handoff should record the bootstrap doctor closeout boundary');
assert.match(docs.finalMaintainerHandoff, /scheduled maintainer doctors are closed through PR #167/, 'final maintainer handoff should record scheduled maintainer doctor closeout');
assert.match(docs.finalMaintainerHandoff, /Weekly default-branch drift check/, 'final maintainer handoff should include weekly commands');
assert.match(docs.finalMaintainerHandoff, /Before release publication review/, 'final maintainer handoff should include before-release commands');
assert.match(docs.finalMaintainerHandoff, /Future work buckets/, 'final maintainer handoff should bucket future work');
assert.match(docs.finalMaintainerHandoff, /When genuine completed inspection evidence arrives/, 'final maintainer handoff should include real-evidence commands');
assert.match(docs.finalMaintainerHandoff, /Stop conditions/, 'final maintainer handoff should include stop conditions');
assert.match(docs.finalMaintainerHandoff, /no open issue rows/i, 'final maintainer handoff should record open issue state');
