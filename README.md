...
Only use `--inspection-evidence <PATH_TO_COMPLETED_REAL_JSON> --attachment-authorization <AUTHORIZATION_RECORD_JSON>` for a completed real package-scoped inspection JSON under `docs/examples/<package>/inspection/*.json` that validates against the inspection evidence contract, has inspector/reviewer/revision provenance, and has explicit Stage 5B human authorization. Do not use `tests/fixtures/inspection-evidence/valid-manual-caliper-inspection.json`, `docs/inspection-evidence-collection/quality-pass-bracket.md`, ignored inbox paths, symlinks, generated CAD quality reports, drawing quality reports, readiness reports, release bundles, screenshots, CI artifacts, or review packs as package inspection evidence.
...
- `npm run test:stage5b:no-evidence`: focused local non-production CLI lane for the Stage 5B no-evidence path across source preflight, intake, promotion dry-run, audit, and surrogate inspection validation
...
| Plain-Python / non-FreeCAD | `dfm`, `review`, `process-plan`, `line-plan`, `quality-risk`, `investment-review`, `readiness-pack`, `readiness-report`, `pack`, `closeout-package`, `inspection-evidence-intake`, `inspection-evidence-promotion-dry-run`, `stage5b-evidence-audit`, `stage5b-evidence-source-kit`, `stage5b-evidence-source-preflight`, `stage5b-surrogate-inspection-validation`, `stabilization-review`, `generate-standard-docs`, `ingest`, `quality-link`, `review-pack`, `review-context`, `compare-rev`, `validate`, `validate-config`, `migrate-config`, `serve` | runs without launching FreeCAD; canonical readiness packaging consumes `review_pack.json`, keeps `readiness_report.json` as the source of truth, and treats `readiness-report <config>` as legacy compatibility rather than the flagship path |
...
`stage5b-evidence-source-kit` and `stage5b-evidence-source-preflight` are local CLI acquisition helpers, not Studio/API jobs and not attachment paths. They write ignored inbox materials and classify raw source readiness for later review while keeping evidence unattached and canonical readiness held.
...
`review-context` accepts explicit package-side evidence inputs for checked-in create quality, drawing quality, drawing QA, drawing intent, feature catalog, and DFM report JSON. It also accepts `--inspection-evidence <path>` for a genuine package-scoped inspection evidence JSON file that passes the inspection evidence contract, but that path now requires `--attachment-authorization <path>` for the Stage 5B human authorization control record with completed redaction/provenance/package-mapping/intake/dry-run/audit reviews. These paths are normalized to portable repo-relative source refs in `review_pack.json`; unsafe outside-repo, ignored output, task-status scratch, ignored Stage 5B candidate inbox paths, and symlinks are not linked as canonical evidence. Quality/drawing/DFM side inputs can close `quality_evidence`, and drawing intent or feature catalogs are recorded as drawing/design-traceability context, but none of these generated side inputs satisfy `inspection_evidence`. Readiness recognizes only the explicit validated and authorized `inspection_evidence` record written into the review pack.
