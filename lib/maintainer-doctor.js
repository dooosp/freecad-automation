...
export const REQUIRED_MAINTAINER_DOCTOR_SCRIPTS = Object.freeze({
  ...
  'maintainer:decision-journal': 'node bin/fcad.js maintainer-decision-journal',
...
export const MAINTAINER_DOCTOR_CHECKS = Object.freeze([
  ...
  Object.freeze({
    id: 'maintainer_decision_journal',
    label: 'Maintainer decision journal',
    mode: 'run',
    argv: Object.freeze([
      'npm',
      'run',
      ...
      'hold',
      '--reason',
      'Maintainer doctor records the audited readiness hold before release checks.',
      '--clean',
    ]),
    detects: Object.freeze(['missing maintainer decision record', 'release exception overclaim', 'readiness proceed overclaim']),
  }),
  Object.freeze({
...
const REQUIRED_DOCS = Object.freeze([
  ...
  'docs/exec-plans/inspection-evidence-onboarding.md',
  'docs/exec-plans/inspection-evidence-onboarding-verification.md',
]);
...
function collectInventory(projectRoot) {
  ...
  return {
    ...
    generated_output_policy: {
      gitignore_paths: ['output/', '.ci/', 'tmp/codex/', 'local/stage5b-candidate-evidence-inbox/', 'local/inspection-evidence-quarantine/']
        .map((path) => ({
          path,
...
function checkGeneratedOutputPolicy(projectRoot, outputDirRel) {
  ...
  const trackedInbox = runGit(projectRoot, ['ls-files', '--', 'local/stage5b-candidate-evidence-inbox', 'local/inspection-evidence-quarantine']);
  const inboxStatus = runGit(projectRoot, ['status', '--short', '--', 'local/stage5b-candidate-evidence-inbox', 'local/inspection-evidence-quarantine']);
  const gitignore = readTextIfExists(projectRoot, '.gitignore');
  const missingGitignoreEntries = ['output/', '.ci/', 'tmp/codex/', 'local/stage5b-candidate-evidence-inbox/', 'local/inspection-evidence-quarantine/']
