...
function formatSchemaErrors(errors = []) {
  return errors.map((error) => {
    ...
    if (error.keyword === 'required' && error.params?.missingProperty) {
      ...
      return `${basePath}${separator}${error.params.missingProperty} is required`;
    }
    return `${basePath} ${error.message}`.trim();
  });
}

function schemaErrors(validator, document) {
  const ok = validator(document);
  return ok ? [] : formatSchemaErrors(validator.errors || []);
}
...
export function validateStage5bIntakeReport(report) {
  return validationResult([
    ...schemaErrors(validateIntakeSchema, report),
    ...validateIntakeSemantics(report),
  ]);
}

export function validateStage5bPromotionDryRunManifest(manifest) {
  return validationResult([
    ...schemaErrors(validatePromotionDryRunSchema, manifest),
    ...validatePromotionDryRunSemantics(manifest),
  ]);
}

export function validateStage5bAuditManifest(manifest) {
  return validationResult([
    ...schemaErrors(validateAuditManifestSchema, manifest),
    ...validateAuditManifestSemantics(manifest),
  ]);
}
...
export function validateStage5bArtifact(document) {
  ...
  return validationResult([`unsupported Stage 5B artifact_type: ${artifactType || 'missing'}`]);
}
