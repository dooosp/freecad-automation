...
const AF_EXECUTION_JOB_CONTRACTS = Object.freeze({
  ...
  'evidence-graph': Object.freeze({
    command: 'evidence-graph',
    layer: 'A+F',
    reentry_target: null,
    canonical_output: 'evidence_graph.json',
  }),
...
function samePathReference(left, right, {
  ...
  if (leftText.startsWith('run/')) {
    if (!portablePathRoot) return false;
    const runRelative = leftText.slice('run/'.length);
    const resolvedRoot = resolve(portablePathRoot);
    const resolvedLeft = resolve(resolvedRoot, runRelative);
    if (!runRelative || !pathWithin(resolvedRoot, resolvedLeft)) return false;
    return resolvedLeft === resolve(rightText);
  }
  if (proof && (leftText.startsWith('input/') || leftText.startsWith('runtime/'))) return false;
  if (proof && isAbsolute(leftText)) return false;
  return resolve(projectRoot, leftText) === resolve(rightText);
}
...
function normalizeSourceArtifactRefs(value) {
  return ensureArray(value)
    ...
    .map((entry) => ({
      ...
      ...(Object.hasOwn(entry, 'sha256') ? { sha256: entry.sha256 } : {}),
      ...(Object.hasOwn(entry, 'size_bytes') ? { size_bytes: entry.size_bytes } : {}),
    }));
}

function extractArtifactPartIdentity(document = {}) {
  const part = isPlainObject(document.part) ? document.part : {};
  const revisionLineage = isPlainObject(document.revision_lineage)
    ? assertRevisionLineage(document.revision_lineage)
    : null;
  return {
    package_slug: normalizeString(
      revisionLineage?.identity?.package_slug || part.package_slug || document.package_slug
    ) || null,
    part_id: normalizeString(
      revisionLineage?.identity?.part_id || part.part_id || document.part_id
    ) || null,
    name: normalizeString(part.name || document.name) || null,
    revision: normalizeString(
      revisionLineage?.identity?.revision || part.revision || document.revision
    ) || null,
    config_sha256: normalizeString(
      revisionLineage?.identity?.config_sha256 || part.config_sha256 || document.config_sha256
    ) || null,
  };
}
...
function normalizeEffectivePolicy(effectivePolicy) {
  ...
  if (!isPlainObject(effectivePolicy) || typeof effectivePolicy.proof_lineage !== 'boolean') {
    throw new AfExecutionContractError(
      'invalid_effective_policy',
      'AF effective_policy.proof_lineage must be an explicit boolean.'
    );
  }
  return Object.freeze({ proof_lineage: effectivePolicy.proof_lineage });
}

function normalizeArtifactBinding(binding) {
  if (binding === null || binding === undefined) return null;
  if (
    !isPlainObject(binding)
    || binding.schema_version !== '1.0'
    || !normalizeString(binding.job_id)
    || !normalizeString(binding.artifact_id)
    || !normalizeString(binding.path)
    ...
    throw new AfExecutionContractError(
      ...
      'AF source artifact binding must contain a registered job, artifact, path, SHA-256, and size.'
    );
  }
  return Object.freeze({
    schema_version: '1.0',
    job_id: normalizeString(binding.job_id),
    artifact_id: normalizeString(binding.artifact_id),
    path: normalizeString(binding.path),
...
export function createAfArtifactIdentityRecord({
  ...
  const record = {
    ...
    ...(revisionLineage ? { revision_lineage: cloneJson(assertRevisionLineage(revisionLineage)) } : {}),
  };
...
export function buildAfArtifactContractMetadata({
  ...
  const normalizedPolicy = normalizeEffectivePolicy(effectivePolicy);
  const normalizedBinding = normalizeArtifactBinding(sourceArtifactBinding);
  return {
    af_contract: {
      contract_version: AF_EXECUTION_CONTRACT_VERSION,
      job_type: jobType,
      ...
      ...(normalizedPolicy ? { effective_policy: cloneJson(normalizedPolicy) } : {}),
      ...(normalizedBinding ? { source_artifact_binding: cloneJson(normalizedBinding) } : {}),
    },
  };
}
...
export function validateDocsManifestAgainstReadiness({
  ...
  if (
    ...
    && !allowBundledPair
    && !readinessRefs.some((ref) => samePathReference(ref.path, readinessPath, {
      projectRoot,
      portablePathRoot,
      proof: docsManifest?.effective_policy?.proof_lineage === true,
    }))
  ) {
