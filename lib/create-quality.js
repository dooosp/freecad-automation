...
const STEP_REIMPORT_HOLE_GEOMETRY_FIXTURE_NAME_PREFIXES = Object.freeze([
...
function isStepReimportHoleGeometryFixture(config = {}) {
  const name = String(config?.name || '').trim();
  return STEP_REIMPORT_HOLE_GEOMETRY_FIXTURE_NAME_PREFIXES.some((fixtureName) => (
    name === fixtureName
    || name.startsWith(`${fixtureName}_runtime_smoke`)
  ));
}
    ...
    return [
      {
        ...
        expected_center_xy_mm: expectedCenterXy,
        status: 'unavailable',
        message: `${geometryLabel} has no cylindrical hole face matching ${requirement.feature_id} within ${centerToleranceMm} mm center tolerance.`,
      },
      {
        ...centerMeasurement,
        status: 'unavailable',
        message: `${geometryLabel} has no cylindrical hole center matching ${requirement.feature_id} within ${centerToleranceMm} mm.`,
      },
    ];
  }
...
function evaluateEngineeringQuality({
  ...
  for (const requirement of requirements) {
    ...
    if (stepRoundtripAttempted && isStepReimportHoleGeometryFixture(config)) {
      ...
      const stepMeasurements = classifyHoleRequirement({
        ...
        shape: shapesById.get(requirement.feature_id),
        cutToolIds,
        cylindricalFaces: normalizeCylindricalFaces(stepRoundtripGeometry?.cylindrical_faces),
        toleranceMm,
        centerToleranceMm,
        ...
        return {
          ...measurement,
          message: `STEP re-import did not produce geometry for fixture-scoped validation of ${requirement.feature_id}.`,
        };
      });
