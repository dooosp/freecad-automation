...
// Cylinder centers can translate along their axis after STEP reimport.
// Only the perpendicular plane identifies the hole center.
function holeFrame(value = [0, 0, 1]) {
  const axis = measurementVector(value);
  if (!axis) return null;
  const length = Math.hypot(...axis);
  if (!(length > 0) || !Number.isFinite(length)) return null;
  if (Math.hypot(axis[0], axis[1]) / length <= 1e-7) return { axis: 'z', depth: 2, indices: [0, 1], plane: 'xy' };
  if (Math.hypot(axis[0], axis[2]) / length <= 1e-7) return { axis: 'y', depth: 1, indices: [0, 2], plane: 'xz' };
  return null;
}

function projectedCenter(value, frame) {
  const vector = measurementVector(value);
  return vector && frame ? frame.indices.map(index => vector[index]) : null;
}
...
function collectExpectedHoleDiameterRequirements(config = {}) {
  const dimensions = Array.isArray(config?.drawing_intent?.required_dimensions)
    ...
    : [];
  const requirements = [];
  const shapes = collectShapesById(config);
  ...
  for (const dimension of dimensions) {
    ...
    for (const featureId of featureRefs.length ? featureRefs : ['unknown']) {
      const frame = holeFrame(shapes.get(featureId)?.direction);
      const plane = frame?.plane || 'xy';
      const centerFields = ['expected_center_xy_mm', 'center_xy_mm', 'expected_center_xz_mm', 'center_xz_mm', 'expected_center_mm', 'center_mm'];
      const suppliedFields = centerFields.filter(key => dimension[key] !== undefined);
      const legacyFields = ['expected_center_xy_mm', 'expected_center_mm', 'center_xy_mm', 'center_mm'];
      const centerField = plane === 'xz' ? suppliedFields[0] : legacyFields.find(key => dimension[key] != null);
      const conflictingPlane = plane === 'xy' && suppliedFields.some(key => key.includes('_xz_'));
      const rawCenter = plane === 'xz' ? dimension[centerField]
        : dimension.expected_center_xy_mm ?? dimension.expected_center_mm ?? dimension.center_xy_mm ?? dimension.center_mm;
      const genericCenter = centerField === 'expected_center_mm' || centerField === 'center_mm';
      const wrongPlane = centerField && !genericCenter && !centerField.includes(`_${plane}_`);
      const expectedCenter = wrongPlane ? null : genericCenter && plane === 'xz'
        ? projectedCenter(rawCenter, frame)
        : measurementVector(rawCenter, plane === 'xz' ? [2] : [2, 3])?.slice(0, 2) || null;
      ...
      requirements.push({
        ...
        invalid_intent: invalidTolerance(rawTolerance) || invalidTolerance(rawCenterTolerance)
          || (plane === 'xz' && suppliedFields.length > 1) || conflictingPlane || wrongPlane || (rawCenter !== undefined && !expectedCenter),
        tolerance_mm: measurementNumber(rawTolerance),
        expected_center_mm: expectedCenter,
        frame,
        expected_center_source_field: expectedCenter
          ? configRequiredDimensionField(dimension.id || `${featureId}_DIA`, centerField)
          : null,
...
function expectedCenterForFeature(shape = null) {
  return projectedCenter(shape?.position, holeFrame(shape?.direction));
}

function expectedCenterForRequirement(requirement, shape = null) {
  return requirement?.expected_center_mm || expectedCenterForFeature(shape);
}
...
function holeFeatureIsAuthoredCutTool({ featureId, shape, cutToolIds }) {
  return Boolean(
    ...
    && holeFrame(shape.direction ?? [0, 0, 1])
  );
}

function faceWithinCutterDepth(face, shape) {
  const depth = holeFrame(shape.direction)?.depth;
  if (depth === undefined || !face.bbox || face.bbox.max[depth] <= face.bbox.min[depth]) return false;
  const start = shape.position[depth];
  const end = start + shape.height * ((shape.direction?.[depth] ?? 1) < 0 ? -1 : 1);
  return face.bbox.min[depth] >= Math.min(start, end) - 1e-7
    && face.bbox.max[depth] <= Math.max(start, end) + 1e-7;
}

// Compare the decimal values carried by JSON/TOML exactly. Squared distances
// avoid sqrt/subtraction roundoff at the boundary without widening tolerance.
function withinMeasurementTolerance(actual, expected, tolerance) {
  if (!Array.isArray(actual) || !Array.isArray(expected) || actual.length !== expected.length) return false;
  const values = [...actual, ...expected, tolerance];
  if (tolerance < 0 || !values.every(value => typeof value === 'number' && Number.isFinite(value))) return false;
  ...
  const integers = decimals.map(value => value.coefficient * (10n ** BigInt(value.exponent - exponent)));
  const distanceSquared = actual.reduce((sum, _, index) => {
    const delta = integers[index] - integers[index + actual.length];
    return sum + delta * delta;
  }, 0n);
  const limit = integers.at(-1);
  return distanceSquared <= limit * limit;
}

function findBestCylindricalFaceMatch({ requirement, shape, expectedCenter, cylindricalFaces, centerToleranceMm }) {
  const frame = holeFrame(shape.direction);
  const authoredCenter = expectedCenterForFeature(shape);
  if (!authoredCenter) return { reason: 'Authored cylinder position is missing or nonfinite' };
  const candidates = cylindricalFaces.filter(face => face.surface_type === 'Cylinder' && holeFrame(face.axis)?.axis === frame?.axis
    && faceWithinCutterDepth(face, shape)
    && withinMeasurementTolerance(projectedCenter(face.center_mm, frame), authoredCenter, centerToleranceMm));
  if (candidates.length !== 1) return {
    reason: candidates.length > 1
      ? 'Multiple cylindrical faces match; split or ambiguous faces are unsupported'
      : `No ${frame.axis.toUpperCase()}-axis cylindrical face matches the authored position and cutter depth`,
  };
  const face = candidates[0];
  return {
    face,
    center_delta_mm: vector2Distance(projectedCenter(face.center_mm, frame), expectedCenter),
...
function collectFaceClaims({ requirements, shapesById, cutToolIds, cylindricalFaces, defaultCenterTolerance }) {
  ...
  for (const requirement of requirements) {
    ...
    const match = findBestCylindricalFaceMatch({
      ...
      expectedCenter: expectedCenterForRequirement(requirement, shape),
...
function baseEngineeringMeasurement({
  ...
  return {
    ...
    ...(requirement.frame?.axis === 'y' ? { hole_axis: 'y', center_plane: 'xz', expected_center_xz_mm: null, actual_center_xz_mm: null } : {}),
...
function classifyHoleRequirement({
  ...
  const expectedCenter = expectedCenterForRequirement(requirement, shape);
  const frame = holeFrame(shape?.direction);
  const expectedField = `expected_center_${frame?.plane || 'xy'}_mm`;
  const actualField = `actual_center_${frame?.plane || 'xy'}_mm`;
  const centerMeasurement = {
    ...centerBase,
    [expectedField]: expectedCenter,
  };
    ...
    return [
      {
        ...diameterBase,
        status: 'unavailable',
        message: `Expected hole feature ${requirement.feature_id} is not a supported Y- or Z-axis cylindrical cut tool in the final shape.`,
      },
      {
        ...centerMeasurement,
        status: 'unavailable',
        message: `Expected hole center for ${requirement.feature_id} is unavailable because the authored feature is not a supported Y- or Z-axis cylindrical cut tool in the final shape.`,
      },
    ];
  }
  ...
  const actualCenter = projectedCenter(face.center_mm, frame);
  const diameterStatus = withinMeasurementTolerance([face.diameter_mm], [requirement.expected_value_mm], toleranceMm) ? 'pass' : 'fail';
  const centerStatus = withinMeasurementTolerance(actualCenter, expectedCenter, centerToleranceMm) ? 'pass' : 'fail';
  ...
  const sourceDelta = generatedDiameter?.actual_value_mm !== null && generatedDiameter?.actual_value_mm !== undefined
    ...
    : null;
  const sourceCenterDelta = generatedCenter?.[actualField]
    ? vector2Distance(actualCenter, generatedCenter[actualField])
    : null;
...
function buildEngineeringMeasurementProvenance({ config = {}, geometry = {} } = {}) {
  ...
  for (const requirement of collectExpectedHoleDiameterRequirements(config)) {
    ...
    const actualCenterField = requirement.frame?.axis === 'y' ? 'actual_center_xz_mm' : 'actual_center_xy_mm';
    records.push(provenanceRecord({
      ...
      report_field: centerMeasurement?.[actualCenterField]
        ? `engineering_quality.measurements[requirement_id=${centerMeasurement.requirement_id}].${actualCenterField}`
        : null,
      source_field: centerMeasurement?.source_field || null,
      value: centerMeasurement?.[actualCenterField] ?? null,
    ...
    if (stepDiameterMeasurement || stepCenterMeasurement) {
      ...
      records.push(provenanceRecord({
        ...
        report_field: stepCenterMeasurement?.[actualCenterField]
          ? `engineering_quality.measurements[requirement_id=${stepCenterMeasurement.requirement_id}].${actualCenterField}`
          : null,
        source_field: stepCenterMeasurement?.source_field || null,
        value: stepCenterMeasurement?.[actualCenterField] ?? null,
