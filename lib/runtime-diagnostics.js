...
import {
  CONDITIONAL_COMMANDS as MANIFEST_CONDITIONAL_COMMANDS,
  DIAGNOSTIC_COMMANDS as MANIFEST_DIAGNOSTIC_COMMANDS,
  FREECAD_BACKED_COMMANDS as MANIFEST_FREECAD_BACKED_COMMANDS,
  PLAIN_PYTHON_COMMANDS as MANIFEST_PLAIN_PYTHON_COMMANDS,
} from '../src/shared/command-manifest.js';

const CHECK_TIMEOUT_MS = 5_000;

export const RUNTIME_DIAGNOSTICS_VERSION = '1.0';
export const RUNTIME_DIAGNOSTICS_HARD_EVIDENCE_RULE = 'Only genuine completed physical/supplier/lab/QA inspection records can satisfy inspection_evidence.';

export const DIAGNOSTIC_COMMANDS = MANIFEST_DIAGNOSTIC_COMMANDS;
export const FREECAD_BACKED_COMMANDS = MANIFEST_FREECAD_BACKED_COMMANDS;
export const PLAIN_PYTHON_COMMANDS = MANIFEST_PLAIN_PYTHON_COMMANDS;
export const CONDITIONAL_COMMANDS = MANIFEST_CONDITIONAL_COMMANDS;
...
const METADATA_FALLBACK_HINT = 'Inspect or repair the CAD model, or provide prebuilt context model_metadata when you need shape-derived metrics.';
...
function redactPathLikeString(value) {
  ...
  return value.replace(/(?:[A-Za-z]:\\|\\\\|\/)[^\s"',;)]+/g, (match) => {
    ...
    return `<path>/${pathLeaf(match)}`;
  });
}

export function redactRuntimeDiagnosticsPaths(value) {
  if (typeof value === 'string') return redactPathLikeString(value);
  if (Array.isArray(value)) return value.map((entry) => redactRuntimeDiagnosticsPaths(entry));
  if (value && typeof value === 'object') {
    return Object.fromEntries(
      Object.entries(value).map(([key, entry]) => [key, redactRuntimeDiagnosticsPaths(entry)])
    );
  }
  return value;
}
...
function buildErrors(runtime, details = {}) {
  const errors = [];
  if (!runtime.available) {
  ...
  if (runtime.available && details.freecad?.error) {
    errors.push(`FreeCAD runtime probe failed: ${details.freecad.error}`);
  }
  return errors;
}

function runtimeProbeStatus(runtime, details = {}) {
  if (!runtime.available) return 'not_detected';
  if (details.freecad?.error) return 'failed';
  return 'usable';
}
...
function normalizeVector(values, fallback = [0, 0, 0]) {
  const items = Array.isArray(values) ? [...values] : [];
  while (items.length < 3) items.push(fallback[items.length] ?? 0);
  return items.slice(0, 3).map((value, index) => normalizeNumber(value, fallback[index] ?? 0));
}

function stepFeaturesToModelMetadata(stepFeatureResult = null) {
  if (!stepFeatureResult || stepFeatureResult.success !== true) return null;

  const bbox = stepFeatureResult.bounding_box || {};
  const size = normalizeVector([bbox.x, bbox.y, bbox.z]);
  const hasSignal = size.some((value) => value > 0)
    || normalizeNumber(stepFeatureResult.volume) > 0
    || normalizeNumber(stepFeatureResult.area) > 0
    || normalizeInt(stepFeatureResult.features?.face_count) > 0
    || normalizeInt(stepFeatureResult.features?.edge_count) > 0;

  if (!hasSignal) return null;

  return {
    volume: normalizeNumber(stepFeatureResult.volume),
    area: normalizeNumber(stepFeatureResult.area),
    faces: normalizeInt(stepFeatureResult.features?.face_count),
    edges: normalizeInt(stepFeatureResult.features?.edge_count),
    vertices: 0,
    bounding_box: {
      min: [0, 0, 0],
      max: [...size],
...
export function hasUsableModelMetadata(modelMetadata = null) {
  if (!modelMetadata || typeof modelMetadata !== 'object') return false;
  const bbox = modelMetadata.bounding_box || {};
  const size = Array.isArray(bbox.size) ? bbox.size : [];
  return size.some((value) => normalizeNumber(value) > 0)
    || normalizeNumber(modelMetadata.volume) > 0
    || normalizeNumber(modelMetadata.area) > 0
    || normalizeInt(modelMetadata.faces) > 0
    || normalizeInt(modelMetadata.edges) > 0
    || normalizeInt(modelMetadata.vertices) > 0;
}

export function buildMetadataOnlyFallbackModelMetadata({ stepFeatureResult = null } = {}) {
  return stepFeaturesToModelMetadata(stepFeatureResult) || {
    volume: 0,
    area: 0,
    faces: 0,
    edges: 0,
...
export function buildModelRuntimeDiagnostic({
  stage,
  modelPath = null,
  message,
  actionableHint = null,
  ...
  return {
    stage: stage || 'model-analysis',
    model_path: modelPath || null,
    message: message || 'Model analysis runtime note.',
    actionable_hint: actionableHint || null,
    fallback_mode: fallbackMode || null,
    details: details || null,
  };
}

export function runtimeDiagnosticToWarning(diagnostic = {}) {
  const pieces = [diagnostic.message].filter(Boolean);
  if (diagnostic.model_path) {
    pieces.push(`Model: ${diagnostic.model_path}`);
  }
  if (diagnostic.fallback_mode === 'metadata-only') {
    pieces.push('Continuing with metadata-only fallback.');
  } else if (diagnostic.fallback_mode === 'no-step-features') {
    pieces.push('Continuing without STEP-derived feature hints.');
  }
  if (diagnostic.actionable_hint) {
    pieces.push(diagnostic.actionable_hint);
  }
  return pieces.join(' ');
}

export function runtimeDiagnosticsToWarnings(diagnostics = []) {
  return diagnostics
    .map((diagnostic) => runtimeDiagnosticToWarning(diagnostic))
    .filter(Boolean);
}

export function defaultMetadataFallbackHint() {
  return METADATA_FALLBACK_HINT;
}
...
export function buildRuntimeDiagnostics({
  ...
  const errors = buildErrors(runtime, details);
  const probeStatus = runtimeProbeStatus(runtime, details);
  const runtimeUsable = probeStatus === 'usable';
  ...
  return {
    ...
    hard_evidence_rule: RUNTIME_DIAGNOSTICS_HARD_EVIDENCE_RULE,
    status: runtimeUsable ? 'ready' : runtime.available ? 'runtime_probe_failed' : 'runtime_not_detected',
    available: runtimeUsable,
    executable_detected: Boolean(runtime.available),
    ...
    next_steps: runtimeUsable ? nextSteps(runtime) : [],
    remediation: runtimeUsable ? [] : remediationSteps(platform, env, runtime),
  };
}
