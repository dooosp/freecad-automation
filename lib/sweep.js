...
export function normalizeSweepSpec(rawSpec = {}) {
  ...
  if (!Array.isArray(jobSource) || jobSource.length === 0) {
    throw new Error('Sweep spec jobs must be a non-empty array');
  }

  const jobs = [...new Set(jobSource.map((entry) => String(entry).trim()).filter(Boolean))];
  ...
  const objectives = isPlainObject(rawSpec.objectives) ? { ...rawSpec.objectives } : {};
  ...
  const execution = isPlainObject(rawSpec.execution) ? { ...rawSpec.execution } : {};
  if (execution.batch_size !== undefined) {
    execution.batch_size = ensureFiniteNumber('execution.batch_size', execution.batch_size);
  }

  return {
    name: typeof rawSpec.name === 'string' && rawSpec.name.trim() ? rawSpec.name.trim() : 'parameter_sweep',
