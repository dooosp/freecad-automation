...
import { validateRevisionLineage } from './revision-lineage-contract.js';
...
const REVISION_LINEAGE_PARENT_BINDINGS = Object.freeze([
  Object.freeze({
...
function lineagePath(path) {
  return path === '/' ? '/revision_lineage' : `/revision_lineage${path}`;
}

function validateRevisionLineageContractWhenPresent(report, errors) {
  if (report.revision_lineage === undefined) return;
  const lineageValidation = validateRevisionLineage(report.revision_lineage);
  lineageValidation.errors.forEach((error) => {
    pushError(
      errors,
      error.code,
      lineagePath(error.path),
      error.message.replace(/^\//, '/revision_lineage/')
    );
  });
  return lineageValidation;
}

function validateRevisionLineageInvariants(report, errors) {
  const lineageValidation = validateRevisionLineageContractWhenPresent(report, errors);
  if (!lineageValidation) return;
  if (!lineageValidation.ok) return;

  const lineage = report.revision_lineage;
  const identity = lineage.identity;
  const candidateIdentity = report.candidate;
  for (const field of ['package_slug', 'part_id', 'revision']) {
    if (candidateIdentity[field] !== identity[field]) {
      pushError(
        errors,
        'conflicting_identity',
        `/candidate/${field}`,
        `/candidate/${field} must equal /revision_lineage/identity/${field}`
      );
    }
  }
  if (candidateIdentity.source_hashes.config !== identity.config_sha256) {
    pushError(
      errors,
      'digest_mismatch',
      '/candidate/source_hashes/config',
      '/candidate/source_hashes/config must equal /revision_lineage/identity/config_sha256'
    );
  }

  const allowedRoles = new Set(REVISION_LINEAGE_PARENT_BINDINGS.map(({ role }) => role));
  if (lineage.parents.length !== REVISION_LINEAGE_PARENT_BINDINGS.length
    || lineage.parents.some((parent) => !allowedRoles.has(parent.role))) {
    pushError(
      errors,
      'conflicting_identity',
      '/revision_lineage/parents',
      '/revision_lineage/parents must contain exactly the baseline/candidate config and review parents'
    );
  }

  REVISION_LINEAGE_PARENT_BINDINGS.forEach(({ role, artifactType, side, sourceKey }) => {
    const matches = lineage.parents.filter((parent) => parent.role === role);
    if (matches.length === 0) {
      pushError(
        errors,
        'missing_parent',
        '/revision_lineage/parents',
        `/revision_lineage/parents must contain role ${role}`
      );
      return;
    }
    if (matches.length !== 1) {
      pushError(
        ...
        'conflicting_identity',
        '/revision_lineage/parents',
        `/revision_lineage/parents must contain exactly one role ${role}`
      );
      return;
    }
    const parent = matches[0];
    if (parent.artifact_type !== artifactType) {
      pushError(
        errors,
        'conflicting_identity',
        '/revision_lineage/parents',
        `${role} must use artifact_type ${artifactType}`
      );
    }
    const expectedHash = report[side].source_hashes[sourceKey];
    if (parent.sha256 !== expectedHash) {
      pushError(
        errors,
        'digest_mismatch',
        '/revision_lineage/parents',
        `${role} digest must equal /${side}/source_hashes/${sourceKey}`
      );
    }
    if (!report[side].artifact_refs.includes(parent.path)) {
      pushError(
        errors,
        'missing_parent',
        '/revision_lineage/parents',
        `${role} path must be retained in /${side}/artifact_refs`
      );
    }
  });
}
...
function validateSemanticInvariants(report, errors) {
  ...
  if (report.baseline.part_id != null
    && report.candidate.part_id != null
    && report.baseline.part_id !== report.candidate.part_id) {
    pushError(
      errors,
      'part_id_mismatch',
      '/candidate/part_id',
      'Baseline and candidate part IDs must match for revision impact analysis'
    );
  }
  validateRevisionLineageInvariants(report, errors);
...
export function validateRevisionImpactReport(report) {
  ...
  if (report && typeof report === 'object' && !Array.isArray(report)) {
    validateRevisionLineageContractWhenPresent(report, errors);
  }
