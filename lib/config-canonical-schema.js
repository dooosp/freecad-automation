...
// _assembly._apply_placement supports Euler [rx, ry, rz] or axis-angle [x, y, z, angle].
const assemblyRotationSchema = {
  type: 'array',
  items: numberSchema,
  minItems: 3,
  maxItems: 4,
...
const productSchema = objectSchema({
  package_slug: stringSchema,
  part_id: stringSchema,
...
const drawingIntentStringOrObjectArraySchema = {
  type: 'array',
  items: {
    anyOf: [
      stringSchema,
...
const drawingIntentSchema = objectSchema({
  part_type: stringSchema,
  manufacturing_process: stringSchema,
  material: stringSchema,
  critical_features: {
    type: 'array',
    items: objectSchema({
  ...
  required_dimensions: {
    ...
    items: objectSchema({
      ...
      feature: stringSchema,
      label: stringSchema,
      dimension_type: stringSchema,
      nominal_mm: nullableNumberSchema,
      value_mm: nullableNumberSchema,
      tolerance: nullableStringSchema,
      required: booleanSchema,
      view: stringSchema,
      reason: stringSchema,
    }),
  },
  required_notes: drawingIntentStringOrObjectArraySchema,
  datum_strategy: objectSchema({
    primary: stringSchema,
    secondary: stringSchema,
    tertiary: stringSchema,
    scheme: stringSchema,
    notes: stringArraySchema,
  }),
  required_views: drawingIntentStringOrObjectArraySchema,
  drawing_standard: {
...
const assemblySchema = objectSchema({
  parts: {
    ...
    items: objectSchema({
      ...
      label: stringSchema,
      position: vector3Schema,
      rotation: assemblyRotationSchema,
      children: {
        type: 'array',
        items: objectSchema(),
      },
...
export const configV1Schema = {
  ...
  properties: {
    ...
    drawing_intent: drawingIntentSchema,
    mounting_reference: objectSchema({
      source: objectSchema({ kind: stringSchema, product_id: stringSchema, evidence_ref: stringSchema }),
      units: stringSchema,
      coordinate_frame: stringSchema,
      coordinate_basis: stringSchema,
      center_tolerance_mm: numberSchema,
      holes: {
        type: 'array',
        items: objectSchema({ id: stringSchema, target_feature_id: stringSchema, center_mm: vector2Schema }),
      },
    }),
    fem: femSchema,
...
export const SUPPORTED_CONFIG_FIELD_GROUPS = [
  ...
  {
    field: 'drawing, drawing_plan, drawing_intent',
    description: 'Drawing metadata, views, style, notes, compiled plan inputs, and optional semantic drawing intent.',
  },
  {
    field: 'fem, tolerance',
