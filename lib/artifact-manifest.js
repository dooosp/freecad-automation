...
export async function collectRuntimeInfo({ runtimeDiagnostics = null } = {}) {
  ...
  return {
    ...
    python_path: runtime.python_executable || process.env.PYTHON || process.env.PYTHON3 || null,
    freecad: runtime
      ? {
          ...
          gui_executable: runtime.gui_executable || '',
          description: runtime.description || '',
          version: runtime.version_details?.freecad?.version ?? null,
          checked_candidates: runtime.checked_candidates || [],
        }
      : null,
  };
}
...
export async function buildArtifactManifest({
  ...
  const runtime = await collectRuntimeInfo({ runtimeDiagnostics });
  const manifest = {
