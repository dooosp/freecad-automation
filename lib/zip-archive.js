...
const DEFAULT_ZIP_ENTRY_DATE = new Date('1980-01-01T00:00:00.000Z');
...
export function buildZipArchive(entries = []) {
  const normalizedEntries = entries.map((entry) => ({
    name: assertSafeEntryName(entry.name),
    data: Buffer.isBuffer(entry.data) ? entry.data : Buffer.from(entry.data || ''),
    date: entry.date instanceof Date ? entry.date : DEFAULT_ZIP_ENTRY_DATE,
  }));
...
export async function createZipArchive(outputPath, entries = []) {
  const archiveBuffer = buildZipArchive(entries);
...
async function readArchiveInput(filePathOrBuffer) {
  if (Buffer.isBuffer(filePathOrBuffer) || filePathOrBuffer instanceof Uint8Array) {
    return {
      buffer: Buffer.from(filePathOrBuffer),
      label: '<immutable ZIP snapshot>',
    };
  }
  return {
    buffer: await readFile(filePathOrBuffer),
    label: String(filePathOrBuffer),
  };
}

export async function listZipEntries(filePathOrBuffer) {
  const { buffer, label } = await readArchiveInput(filePathOrBuffer);
  const eocdOffset = findEndOfCentralDirectory(buffer);
  if (eocdOffset < 0) {
    throw new Error(`Invalid ZIP archive: could not find end-of-central-directory in ${label}`);
  }
...
export async function readZipEntry(filePathOrBuffer, entryName) {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (name === normalizedEntryName) {
      if (method !== ZIP_STORE_METHOD) {
        throw new Error(`ZIP entry ${normalizedEntryName} in ${label} uses unsupported compression method ${method}`);
      }
      ...
      if (localName !== name) {
        throw new Error(`Invalid ZIP archive: local/central entry name mismatch for ${normalizedEntryName}`);
      }
      if (localFlags !== flags
        || localMethod !== method
        ...
        throw new Error(`Invalid ZIP archive: local/central metadata mismatch for ${normalizedEntryName}`);
      }
      const dataOffset = localHeaderOffset + 30 + localNameLength + localExtraLength;
      if (dataOffset + compressedSize > buffer.length) {
        throw new Error(`Invalid ZIP archive: truncated data for ${normalizedEntryName}`);
      }
      const data = buffer.subarray(dataOffset, dataOffset + compressedSize);
      if (data.length !== uncompressedSize || crc32(data) !== expectedCrc) {
        throw new Error(`Invalid ZIP archive: CRC or size mismatch for ${normalizedEntryName}`);
      }
  ...
  throw new Error(`ZIP entry not found in ${label}: ${normalizedEntryName}`);
}
