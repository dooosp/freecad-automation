...
export function buildLandingPayload({
  ...
  return sanitizePublicPayload({
    ...
    studio: {
      ...
      note: 'Preview routes stay scratch-safe and request/response. Studio tracked routes cover create/draw/inspect/report, source-path review-context handoff, and compare, readiness, stabilization, docs, and pack follow-up; direct POST /jobs accepts the same AF continuation job types.',
    },
    api_info: {
      available: true,
      path: '/api',
...
function buildLandingCopy(locale = 'en') {
  if (locale === 'ko') {
    return {
      ...
      trackedJobs: 'Studio TOML, source-path review-context 핸드오프, 또는 산출물 참조에서 생성/도면/검사/리포트와 비교, 준비 상태, 안정화 검토, 문서, 팩 후속 작업을 대기열에 넣고, readiness 기반 문서 작업은 필요 시 config 유사 입력을 자동 복구합니다',
      ...
      reentryDetailsHtml: '추적 <code>report</code>는 설정 형태의 <code>artifact_ref</code>를 받고, 추적 <code>inspect</code>는 모델 형태의 <code>artifact_ref</code>를 받습니다. 추적 <code>review-context</code>는 source path 핸드오프로 대기열에 넣습니다. 추적 <code>readiness-pack</code>은 정식 review-pack에서, <code>generate-standard-docs</code>와 <code>pack</code>은 정식 readiness-report 또는 <code>release_bundle.zip</code>에서 이어갈 수 있고, <code>compare-rev</code>와 <code>stabilization-review</code>는 기준선/후보 정식 산출물 쌍이 모두 있을 때만 준비됩니다.',
  ...
  return {
    ...
    reentryDetailsHtml: 'tracked <code>report</code> accepts config-like <code>artifact_ref</code>, tracked <code>inspect</code> accepts model-like <code>artifact_ref</code>, and tracked <code>review-context</code> is queued from source-path handoff. Tracked <code>readiness-pack</code> continues from canonical review-pack inputs; tracked <code>generate-standard-docs</code> and <code>pack</code> continue from canonical readiness-report inputs or <code>release_bundle.zip</code>, while <code>compare-rev</code> and <code>stabilization-review</code> require baseline/candidate canonical artifact pairs.',
